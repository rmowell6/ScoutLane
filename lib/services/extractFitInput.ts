...
const EXTRACT_INSTRUCTIONS = [
  ...
  '  requirement sentences. Take these from the JD’s required / minimum-qualifications section only.',
  '- preferredSkills: the JD’s PREFERRED / nice-to-have / "bonus" skills as the SAME kind of short',
  '  canonical tokens. Many JDs list these in a separate "Preferred" or "Nice to have" section (e.g.',
  '  Terraform, Docker, Kubernetes, Python, PostgreSQL). Capture each as its own token; never duplicate',
  '  a token already in mustHaveSkills. Empty array if the JD lists no preferred skills.',
  '- candidateSkills: for EVERY must-have OR preferred token the candidate genuinely has, include that',
  '  EXACT SAME token (verbatim) here; then add any other real skills. Do not reword a must-have or',
  '  preferred token when the candidate has it — reuse the identical string so it matches.',
  ...
  'engagementType = the JD’s engagement/tax structure: w2_fte (permanent employee), w2_contract (W2',
  'contractor), c2c (corp-to-corp), c2c_1099 (1099/independent). Use "unspecified" unless the JD',
  'states it explicitly (e.g. "W2 only", "no C2C", "1099 contract"). Do NOT infer from tone.',
  'sponsorshipAvailable = "no" ONLY when the JD explicitly says it will not sponsor (e.g. "unable to',
  'sponsor visas", "must be authorized to work without sponsorship"); "yes" only when it explicitly',
  'offers sponsorship; "unspecified" otherwise. MOST postings are "unspecified"; never guess "no".',
  'evidence = for roleTypeMatch, seniorityMatch, employerType, location, vertical, engagementType, and',
  'sponsorshipAvailable, return a SHORT VERBATIM excerpt (fewer than 15 words) copied EXACTLY from the',
  'JD that supports each classification. Copy the JD text word for word, do not paraphrase; use an',
  'empty string when the JD offers no direct support. These quotes are used only to verify grounding;',
  'for engagementType and sponsorshipAvailable a missing/unmatched quote resets the field to',
  '"unspecified" (no penalty); for the others it only sets a non-blocking flag. They never invent a score.',
