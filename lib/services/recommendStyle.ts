 ...
 * failure so the packet still ships.
 */
export async function recommendStyle(profile: Profile, jobReqs: JobReqs): Promise<StyleRecommendation> {
  try {
    ...
    const signals = readParsed(message, 'recommendStyle', MAX_TOKENS)
